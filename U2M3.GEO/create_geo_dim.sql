...
SELECT
    ...
    Count(a.parentall) over(Partition by a.root) child_geo_sys,
    ...
    Count(a.parent_geo_id) over(Partition by a.parentall) child_geo_parts,
  ...
  --  nvl(gp.localization_id, 0)               localization_id,
    ...
    Count(a.child_geo_id) over(Partition by a.parent_geo_id) child_geo_regions,
...
UNION
    ...
    sys_guid() dim_geo_id,
    'geo_group'                              code,
    Count(x.parentall) over(Partition by x.root) child_geo_sys,
    ...
    Count(x.parent_geo_id) over(Partition by x.parentall) child_geo_parts,
  ...
  --  nvl(gp.localization_id, 0)               localization_id,
    ...
    Count(x.child_geo_id) over(Partition by x.parent_geo_id) child_geo_regions,
...
select * from u_dw_dim_tax.dim_geo_obj_scd;